 ...
 *
 * NOTA TÉCNICA:
 * Este código es el resultado de corregir errores de kimi LLM en el experimento
 * https://vibecodingmexico.com/organizador-de-ias/
 *
 * Usalo bajo tu propio riesgo, son cambios menores lo que hice pero NO
 * está revisado a fondo
 *
...
define('PASS_MAESTRA', 'tupass'); // para agregar, editar y borrar
define('PASS_REGISTROS', 'tu_contrasena_registros_default'); // contraseña default para registros protegidos (opcional)
define('IPS_PERMITIDAS', ['127.0.0.1', '192.168.1.1','::1']); // agrega tus IPs aquí

// Configuración de base de datos
define('DB_HOST', 'localhost');
define('DB_USER', 'eluser');
define('DB_PASS', 'elpass');
define('DB_NAME', 'labase');
...
if ($action === 'save' && $_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($tipo === 'imagen' && !validarBase64Imagen($contenido) && !empty($contenido)) {
        ...
        if ($id > 0) {
            ...
            $stmt->bind_param("sssssdssdsssssi", 
                $proyecto, $ia_utilizada, $tipo, $contenido,
                $nombre_archivo, $num_version, $comentarios,
			...
			*/
			// orregido
			// SQL: 1.proy, 2.ia, 3.tipo, 4.cont, 5.arch, 6.ver, 7.com, 8.calif, 9.vis, 10.tam, 11.md5, 12.sha1, 13.pass(IF), 14.pass(IF), 15.id
$stmt = $mysqli->prepare("UPDATE ai_backups SET 
    proyecto = ?, ia_utilizada = ?, tipo = ?, contenido = ?, 
    nombre_archivo = ?, num_version = ?, comentarios = ?, 
    calificacion = ?, visible = ?, tamanio = ?, hash_md5 = ?, hash_sha1 = ?,
...
// Cadena de tipos (15 caracteres exactos, sin espacios):
// s (proy), s (ia), s (tipo), s (cont), s (arch), d (ver), s (com), d (calif), s (vis), d (tam), s (md5), s (sha1), s (pass), s (pass), i (id)
$stmt->bind_param("sssssdsdsdssssi", 
    ...
    $comentarios,       // 7
    $calificacion,      // 8 (d)
    $visible,           // 9 - AQUÍ IRÁ EL "SI"
    $hashes['tamanio'], // 10 (d)
    $hashes['md5'],     // 11
    $hashes['sha1'],    // 12
    $contrasena_ver,    // 13 (para el IF)
    $contrasena_ver,    // 14 (para el IF)
    $id                 // 15 (i)
);
			// fincorregido
        } else {
